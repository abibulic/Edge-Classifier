...
def _concatenate_data_dicts(data_dicts):
  ...
  for field in concatenated_dicts:
    if field in (RECEIVERS, SENDERS):
      concatenated_dicts[field] = torch.as_tensor(concatenated_dicts[field]).type(torch.LongTensor).to(torch.device('cuda:0'))
    else:
      concatenated_dicts[field] = torch.as_tensor(concatenated_dicts[field]).to(torch.device('cuda:0'))
